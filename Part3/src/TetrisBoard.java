...
public class TetrisBoard {
	...
	/**
	 * indicate when the level has been lost when a block is above the top of
	 * the board
	 */
	public boolean gameEnded() {
		...
		int row = 0;
		for (int i = 0; i < NUM_COLS; i++) {
			// if there is one position where there is a block
			if (blockMatrix[row][i] == true) {
				// the game is over
				return true;
			}
		}
